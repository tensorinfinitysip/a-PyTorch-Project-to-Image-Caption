...
def save_checkpoint(save_dir, epoch, epochs_since_improvement, encoder, decoder, encoder_optimizer, decoder_optimizer,
    ...
    :param epochs_since_improvement: number of epochs since last improvement in top5 accuracy
    ...
    filename = '_'.join((str(state['epoch']), filename))
    filepath = os.path.join(save_dir, filename)
    mkdir_if_missing(save_dir)
    torch.save(state, filepath)
    # If this checkpoint is the best so far, store a copy so it doesn't get overwritten by a worse checkpoint
    if is_best:
        shutil.copy(filepath, os.path.join(save_dir, 'BEST_MODEL.pth.tar'))
